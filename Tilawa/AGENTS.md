...
- `:shared`: KMP library module containing shared logic for Android, iOS, macOS, Linux, and Windows targets. C-export façade (`shared_*` symbols, `SharedCApi.kt`) lives in `cApiMain` (shared by `mingwX64Main`/`linuxX64Main`); after changing exports run `.agents/scripts/abi-export.sh` and keep `abi/` snapshots green via `.agents/scripts/abi-check.sh`.
